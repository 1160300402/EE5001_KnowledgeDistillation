...
affine_par = True
# BatchNorm2d = functools.partial(InPlaceABNSync, activation='none') #3.22.2022 change
...
class PSPModule(nn.Module):
    ...
    def __init__(self, features, out_features=512, sizes=(1, 2, 3, 6)):
        ...
        self.bottleneck = nn.Sequential(
            ...
            # InPlaceABNSync(out_features), #3.23.2022 change
            nn.BatchNorm2d(out_features), #3.23.2022 change
            nn.ReLU(inplace=True), #3.23.2022 change
            nn.Dropout2d(0.1)
            )
    ...
    def _make_stage(self, features, out_features, size):
        ...
        # bn = InPlaceABNSync(out_features) #3.23.2022 change
        bn = nn.BatchNorm2d(out_features) #3.23.2022 change
        re = nn.ReLU(inplace=True) #3.23.2022 change
    ...
    def __init__(self, block, layers, num_classes):
        ...
        if layers == [3, 4, 23, 3]:
            ...
            self.dsn = nn.Sequential(
                ...
                #InPlaceABNSync(512), #3.23.2022 change
                nn.BatchNorm2d(512), #3.23.2022 change
                #nn.ReLU(inplace=True), #3.23.2022 change
                nn.Dropout2d(0.1),
            ...
            self.dsn = nn.Sequential(
                ...
                #InPlaceABNSync(128),
                nn.BatchNorm2d(128), #3.23.2022 change
                #nn.ReLU(inplace=True), #3.23.2022 change 加上之后和原来的premodel 结果不同了
                nn.Dropout2d(0.1),
