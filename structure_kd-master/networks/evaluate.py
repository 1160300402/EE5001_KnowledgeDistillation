...
def evaluate_main(model, loader, gpu_id, input_size, num_classes, whole = False, recurrence = 1, type = 'val'):
    ...
    for index, batch in enumerate(loader):
        ...
        if type == 'val':
            ...
            seg_gt = seg_gt[ignore_index] #(1024,2048)
        ...
        if (index % 50 == 0):
            ...
            print('Step: [{}/{}] '
                        'Mean_IU {:.4f}.'.format(index + 1, len(loader),mean_IU))
...
def evaluate_nyudv2(model, loader, gpu_id, input_size, num_classes, whole = False, recurrence = 1, type = 'val'):
    """Create the model and start the evaluation process."""
    ...
    for index, batch in enumerate(loader):
        ...
        # output_im.save('outputs/'+name[0]+'.png')
        if type == 'val':
            seg_gt = np.asarray(label[0].numpy()[:h,:w], dtype=np.int)
            ignore_index = seg_gt != 255
...
def predict_sliding_test(net, image, tile_size, classes, flip_evaluation, recurrence):
    interp = nn.Upsample(size=tile_size, mode='bilinear', align_corners=True)
    image_size = image.shape
    overlap = 1/3
    ...
    for row in range(tile_rows):
        for col in range(tile_cols):
            ...
            padded_img = pad_image(img, tile_size)
            tile_counter += 1
            padded_prediction = net(Variable(torch.from_numpy(padded_img), volatile=True))
            if isinstance(padded_prediction, list):
                padded_prediction = padded_prediction[0]
            padded_prediction = interp(padded_prediction).cpu().data[0].numpy().transpose(1,2,0)
            prediction = padded_prediction[0:img.shape[2], 0:img.shape[3], :]
...
if __name__ == '__main__':
    # restore_from = './snapshots/nyudv2_2S_400_0.37867405898731815.pth'
    ...
    # student.load_state_dict(torch.load(restore_from))
    # mean_IU, IU_array = evaluate_nyudv2(student, valloader, '0', '480,640', 14, False)
    # print('[nyudv2 val 480,640] mean_IU:{:.6f} IU_array:{}'.format(mean_IU, IU_array))
    # logging.info('[nyudv2 val 480,640] mean_IU:{:.6f} IU_array:{}'.format(mean_IU, IU_array))

    restore_from = './snapshots/nyudv2_3T_1200_0.41842759912941635.pth'
    valloader = data.DataLoader(NYUv2Loader(root="./dataset/nyu_dv2", split='val', is_transform=True),
                                batch_size=1, shuffle=False, pin_memory=True)
    teacher = Res_pspnet(Bottleneck, [3, 4, 23, 3], num_classes=14)
    teacher = torch.nn.DataParallel(teacher)
    teacher.load_state_dict(torch.load(restore_from))
    mean_IU, IU_array = evaluate_nyudv2(teacher, valloader, '0', '480,640', 14, False)
    print('[nyudv2 val 480,640] mean_IU:{:.6f} IU_array:{}'.format(mean_IU, IU_array))
    logging.info('[nyudv2 val 480,640] mean_IU:{:.6f} IU_array:{}'.format(mean_IU, IU_array))
