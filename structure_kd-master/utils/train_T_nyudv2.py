...
from utils.augmentations import Compose, RandomHorizontallyFlip, RandomRotate, RandScale, RandomGaussianBlur, \
    CenterCrop, RandomSizedCrop, RandomSized, Scale
...
class nyudv2_trainmodel():
    ...
    def __init__(self, args):
        ...
        trainloader = data.DataLoader(
            NYUv2Loader(root="./dataset/nyu_dv2", is_transform=True, is_augment=True, augmentations=train_transform,
                        img_size=(256, 256)),
    ...
    def train(self, model):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if (epoch_log % args.save_freq == 0) or (epoch == args.epochs - 1):
                mean_IU, IU_array = evaluate_nyudv2(model, self.valloader, '0', '480,640', 14, False)
                mean_IU_train, IU_array_train = evaluate_nyudv2(model, self.trainloader, '0', '256,256', 14, False)
    ...
    def save_model(self, model, epoch, mean_IU):  # 只存放model
        torch.save(model.state_dict(),
                   osp.join(self.args.snapshot_dir,
                            'nyudv2_5e-3' + str(self.args.train_type) + '_' + str(epoch) + '_' + str(mean_IU) + '.pth'))
