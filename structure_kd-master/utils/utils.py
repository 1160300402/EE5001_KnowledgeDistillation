...
def load_T_model(model, ckpt_path):
    ...
    if os.path.exists(ckpt_path):
        ...
        # saved_state_dict = torch.load(ckpt_path,map_location='cpu') #3.22.2022 姚远修改
...
def load_S_model(args, model, with_module=True):
    ...
    if args.is_student_load_imgnet:
        ...
        if args.S_resume:
            ...
            file = args.S_ckpt_path + args.S_ckpt_name
            if os.path.isfile(file):
                ...
                best_IU_array = checkpoint['IU_array'] if 'IU_array' in checkpoint else {}
